...
namespace Web_Service.Controllers
{
    ...
    {
        private readonly BridgesdbMainContext db;
        private string datanumber = "2069",
                       api_key = "bd78f248342dc0921a9447a36393f6f5";

        public DataController(BridgesdbMainContext context)
        {
            db = context;
        }
        ...
        {
            return Ok(JWriter<List<BridgeInfo>>.Write(await db.Bridges.ToListAsync()));
        }
        ...
        public async Task<IActionResult> Create() 
        {
            try
            {
                ...
                var Deser_Obj = JDeserializer<List<Bridge>>.Deser(data);

                db.Bridges.RemoveRange(db.Bridges);
        ...
        {
            int current_count = await db.Bridges.CountAsync(), count = 0;
        ...
        public async Task<IActionResult> Export()
        {
            try
            {
                ...
                {
                    sw.WriteLine(JWriter<List<BridgeInfo>>.Write(await db.Bridges.ToListAsync()));
                }
            }
            catch (Exception ex)
            {
